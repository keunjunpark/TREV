...
```python
# QAOA for MaxCut
def create_qaoa_circuit(num_qubits, edges, p_layers):
    ...
    for p in range(p_layers):
        # Problem unitary: exp(-i γ C) via ZZ interactions on edges
        for (i, j), _ in edges:
            circuit.cx(i, j)
            circuit.rz(j)      # γ parameter
            circuit.cx(i, j)
        
        # Mixer unitary: exp(-i β B) with X rotations
        for i in range(num_qubits):
...
from TREV.utils.maxcut import gengraph, create_hamiltonian
edges = gengraph(6)
ham_terms = create_hamiltonian(6, edges)
hamiltonian = Hamiltonian(num_qubits=6)
for coeff, pauli_tuple in ham_terms:
...
qaoa_circuit = create_qaoa_circuit(6, edges, p_layers=3)
